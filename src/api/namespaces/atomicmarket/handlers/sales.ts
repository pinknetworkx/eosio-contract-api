...
export async function getSaleLogsAction(params: RequestValues, ctx: AtomicMarketContext): Promise<any> {
    const args = filterQueryArgs(params, {
        ...
        limit: {type: 'int', min: 1, max: Math.min(ctx.serverConfig.per_page_limits?.market?.logs || 100), default: 100},
...
export async function getSalesAction(params: RequestValues, ctx: AtomicMarketContext): Promise<any> {
    const args = filterQueryArgs(params, {
        page: {type: 'int', min: 1, default: 1},
        limit: {type: 'int', min: 1, max: Math.min(ctx.serverConfig.per_page_limits?.market?.sales || 100), default: 100},
...
export async function getSalesTemplatesAction(params: RequestValues, ctx: AtomicMarketContext): Promise<any> {
    const args = filterQueryArgs(params, {
        ...
        page: {type: 'int', min: 1, default: 1},
        limit: {type: 'int', min: 1, max: Math.min(ctx.serverConfig.per_page_limits?.market?.sales_templates || 100), default: 100},
        sort: {
            type: 'string',
