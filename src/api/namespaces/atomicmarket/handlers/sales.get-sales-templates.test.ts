...
import { getSalesTemplatesAction } from './sales';
...
async function getSalesIds(values: RequestValues): Promise<Array<number>> {
    ...
    const result = await getSalesTemplatesAction({
        symbol: 'TEST',
        ...values,
    }, testContext);
...
describe('AtomicMarket Sales API', () => {
    ...
    describe('getSalesTemplatesAction', () => {
        ...
        txit('only returns the first asset in a bundle', async () => {
            // in new version, ignore bundles
            const {template_id} = await initTest();

            const {asset_id, offer_id} = await client.createOfferAsset({}, {template_id});

            const {asset_id: asset_id2} = await client.createOfferAsset({offer_id, index: 2}, {
                template_id: (await client.createTemplate()).template_id,
            });

            const {sale_id} = await client.createSale({offer_id});

            expect(await getSalesIds({asset_id: `${asset_id}, ${asset_id2}`}))
                .to.deep.equal([sale_id]);
        });
        ...
        txit('formats and fills result', async () => {
            ...
            const [result] = await getSalesTemplatesAction({symbol: 'TEST', burned: false}, testContext);
