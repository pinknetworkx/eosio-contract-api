CREATE OR REPLACE PROCEDURE update_neftymarket_auction_mints(selected_contract TEXT, last_irreversible_block BIGINT, max_auctions_to_update INT = 50000)
LANGUAGE plpgsql
AS $$
DECLARE
    r RECORD;
    ...
    FOR r IN
        WITH auctions_to_update AS MATERIALIZED (
            ...
            FROM auctions_to_update auction
                JOIN neftymarket_auctions_assets asset ON (auction.market_contract = asset.market_contract AND auction.auction_id = asset.auction_id)
        ...
        FROM new_mints
    LOOP
        UPDATE neftymarket_auctions auction
            SET template_mint =
